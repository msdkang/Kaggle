...
class load_data:
    def build_query(filename: str, encoding = None) -> str:
        '''
        `build_query` loads a sql query into `str`.
        ...
        Notes
        -----
        - query file can be in any text fomrat: `.txt` or `.sql`
        '''
    ...
    def execute_query_old(qry: str, 
                    server_name: str = 'edwsqlcl') -> pd.DataFrame:
        '''
        `execute_query` executes a SQL query and returns data as a `DataFrame`.

        Parameters
        ----------
        qry: `str`
            SQL query 
        server_name: `str`, default = EDWSQLCL
            server name
        ...
        Notes
        -----
        - it can execute only one query
        '''
        connection_str = f"DRIVER={{SQL Server Native Client 11.0}};SERVER=" + \
            server_name + ";Trusted_Connection=yes;"
        ...
        try:
            ...
            print("trying to execute {} ...".format(qry[:40]))
            df = pd.read_sql(qry, conn)
            print("{} records are loaded successfully using {:.2f} mB".format(
                df.shape[0], 
                df.memory_usage().sum() / 1000000))
        except pyodbc.Error as ex:
            print("unable to execute query due to : {}".format(ex.args[1]))
        finally:
            conn.close()
            print("connection to server is closed successfully.")
        return df

    def execute_query(qry: str, 
                    server_name: str = 'edwsqlcl',
                    parameters: list[object] = []) -> pd.DataFrame:
        '''
        `build_query` loads a sql query into `str`.
        ...
        try:
            ...
            print("trying to execute {} ...".format(qry[:40]))
            result = cursor.execute(qry, parameters)
            ...
            print("{} records are loaded successfully.".format(
                df.shape[0]))
    ...
    def execute_mulyiple_queries(queries: list[str], 
                            server_name: str = 'edwsqlcl') -> pd.DataFrame:
        '''
        `execute_mulyiple_queries` executes a list of SQL querys and returns data \
            as a `DataFrame`.
        ...
        Notes
        -----
        - it can return only one data table
        '''
        connection_str = f"DRIVER={{SQL Server Native Client 11.0}};SERVER=" + \
            server_name + ";Trusted_Connection=yes;"

        print("trying to connect to server ...", end="\r")
        try:
            conn = pyodbc.connect(connection_str)
            print("connected to server successfully.")
            cursor = conn.cursor()
            ...
            for qry in queries:
                print("trying to execute {} ...".format(qry[:40]), end="\r")
